...
public class Juego extends InterfaceJuego {
    ...
    private boolean personajeColisionó = false;
    ...
    public void tick() {
        // Temporizador
        if(tiempoRestante > 0 && !personajeColisionó) {
        ...
        // Dibuja al Prota
        if (prota != null && prota.getImagen() != null && !personajeColisionó) {
     ...
     // Mueve y dibuja las Tortugas
        for (int i = 0; i < tortugas.length; i++) {
            ...
            if (tortuga != null) {
                ...
                if (tortuga.colisionaCon(prota) && !personajeColisionó) {
                ...
                for (Plataforma p : plataformas) {
                    if (p.getY() != 100 && p.colisionaCon(tortuga)) { // Omite la primera plataforma
                        tortuga.detenerCaida(p.getY() - 25);
                        tortuga.setLimites(p.getX() - 60, p.getX() + 60);
                        break;
                    }
                }
            }
        ...
        for (int i = 0; i < gojos.length; i++) {
            ...
            if (gojo != null) {
                ...
                // Verifica colisión con el Prota
                if (gojo.colisionaCon(prota) && !personajeColisionó) {
     ...
     // Mueve y dibuja la bolita
        ...
        if (tiempoRestante <= 0 || personajeColisionó) {
            ...
            if ((System.currentTimeMillis() / 500) % 2 == 0) {
                this.entorno.cambiarFont("PixelatedFont", 36, java.awt.Color.WHITE);
                this.entorno.escribirTexto("GAME OVER", 300, 300);
                this.entorno.cambiarFont("Georgia", 18, java.awt.Color.WHITE);
                this.entorno.escribirTexto("Presiona 'R' para reiniciar", 300, 350);
            }
            
    
            // Espera a que se presione la tecla 'R'
            if (this.entorno.seLevanto('r')) {
                reiniciarJuego();
            }
        }
    }
    ...
    private void reiniciarJuego() {
        prota = new Prota(14, 450);
        tortugas = new Tortuga[5];
        gojos = new Gojos[5];
        marcadorTortugas = 0;
        marcadorGojos = 0;
        gojosPerdidos = 0;
        tiempoRestante = 90;   
        personajeColisionó = false;
    }
